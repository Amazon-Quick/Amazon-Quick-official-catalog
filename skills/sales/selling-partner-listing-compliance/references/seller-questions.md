---
description: "The grouped question template for Step 4 of the compliance pre-flight: which facts to ask the seller for, organized by the regulator bucket that makes them relevant, so the agent asks once and never infers a compliance fact."
last_updated: 2026-09-16
origin: original
---

# Seller questions (Step 4)

Ask **once**, grouped, and include only the blocks that Step 3 classification made relevant. Use the structured question tool where available; otherwise a single message with numbered items. Record every answer as met, unmet, or not applicable. "I don't know" is unmet.
...
For each item record `met` (seller confirmed and, where relevant, has the document), `unmet` (missing, or "I don't know"), or `n/a` (bucket does not apply). The checklist in Step 8 is built from these states plus the Seller Assistant confirmation and the gating result. Never fill a value the seller did not give.
